...
const TagsList = () => {
    const { tags, removeTag } = useSetting()
    const[newTag, setNewTag] = useState<boolean>(false)
    const[tag, setTag] = useState<SettingItems | null>(null)
    ...
    const endSend = (type: string) => {
        ...
        if(type === 'add' || type === 'edit') {
            ...
            type: "success",
            show: true,
            text: type === 'add' ? 'اضافه کردن دسته بندی'  : 'ویرایش دسته بندی'
        })
        }
    }
        ...
        <>
            <div className="h-full">
                <div className="w-full flex items-center justify-between pb-4 mb-4 border-b-1 border-gray-200">
                    ...
                    <Button color="green" name="دسته بندی جدید" icon="add" onClick={() => setNewTag(true)} cancel={false} />
                </div>
                    ...
                    <table className="table text-sm w-full border-separate border-spacing-y-4">
                        <thead className="sticky top-0 bg-white z-10">
                            <tr className="text-right">
                                <th className="p-2 text-sm text-gray-500 border-b-1 border-gray-200 font-normal">عنوان دسته بندی</th>
                            ...
                            {tags &&  tags.length > 0 ? tags?.map((item) => (
                                <tr key={item.id} style={{boxShadow: "0 0 30px #0000000d"}} className="text-right border-1 border-gray-200 rounded">
                                    ...
                                    <th className="p-3 w-50">
                                        {tagIcons.find(i => i.id === item.icon)?.value}
                                    </th>
                                    <th className="p-3 flex items-center gap-2">
                                        <button onClick={() => {setTag(item ? item : null); setNewTag(true); setIsEdit(true)}} className="flex items-center justify-center p-2 rounded bg-blue-600 hover:bg-blue-800 text-white cursor-pointer">
                                        ...
                                        <button onClick={() => {
                                            setDeleteItem(item)
                                            setDeleteModal(true)
                                        }} className="flex items-center justify-center p-2 rounded bg-red-500 hover:bg-red-700 text-white cursor-pointer">
            ...
            {newTag && (
                <AddAndEditTag endSend={endSend} isEdit={isEdit} item={tag || null} onClose={() => {setNewTag(false); endSend('')}} />
            )}
