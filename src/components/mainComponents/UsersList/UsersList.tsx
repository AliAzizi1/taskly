...
import AddAndEditUser from "@/app/users/AddAndEditUser/AddAndEditUser"
...
const UsersList = () => {
    const { removeUser, users, } = useSetting()

    const[newUser, setNewUser] = useState<boolean>(false)
    const[editUser, setEditUser] = useState<SettingItems | null>(null)
    ...
    const endSend = (type: string) => {
        ...
        if(type === 'add' || type === 'edit') {
            setShowMessage({
                type: "success",
                show: true,
                text: type === 'add' ? 'اضافه کردن کاربر'  : 'ویرایش کاربر'
            })
        }
    }
        ...
        <>
            <div className="h-full">
                <div className="w-full flex items-center justify-between pb-4 mb-4 border-b-1 border-gray-200">
                    <span className="font-bold">همه کاربر ها</span>
                    <Button color="green" name="کاربر جدید" icon="add" onClick={() => setNewUser(true)} cancel={false} />
                </div>
                    ...
                    <table className="table text-sm w-full border-separate border-spacing-y-4">
                        <thead className="sticky top-0 bg-white z-10">
                            <tr className="text-right">
                                <th className="p-2 text-sm text-gray-500 border-b-1 border-gray-200 font-normal">نام کاربر</th>
                            ...
                            {users &&  users.length > 0 ? users?.map((item) => (
                                <tr key={item.id} style={{boxShadow: "0 0 30px #0000000d"}} className="text-right border-1 border-gray-200 rounded">
                                    ...
                                    <th className="p-3 w-50">
                                        {userIcons.find(i => i.id === item.icon)?.value}
                                    </th>
                                    <th className="p-3 flex items-center gap-2">
                                        <button onClick={() => {setEditUser(item); setNewUser(true); setIsEdit(true)}} className="flex items-center justify-center p-2 rounded bg-blue-600 hover:bg-blue-800 text-white cursor-pointer">
            ...
            {newUser && (
                <AddAndEditUser endSend={endSend} isEdit={isEdit} item={editUser || null} onClose={() => {setNewUser(false); endSend('exit')}} />
            )}
